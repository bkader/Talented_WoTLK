...
do
    ...
    function Talented:UpdateTemplateName(template, newname)
        if self.db.global.templates[newname] or template.talentGroup or type(newname) ~= "string" or newname == "" then
            return
        end
...
do
    ...
    function Talented:PackTemplate(template)
        if not template or template.talentGroup or template.code then
            return
        end
...
do
    ...
    function TalentView:SetClass(class, force)
        if self.class == class and not force then
            return
        end
    ...
    function TalentView:UpdateTalent(tab, index, offset)
        ...
        if offset > 0 and Talented:IsTemplateAtCap(template) then
            return
        end
...
do
    ...
    function Talented:PET_TALENT_UPDATE()
        ...
        if not class or not PetTalentsAvailable() then
            return
        end
...
do
    ...
    function Talented:ExportWhpetTemplate(template, url)
        if RAID_CLASS_COLORS[template.class] then
            return
        end
